...
function value(tr,col){
 ...
 if(col===4){const raw=td.dataset.sortDate||td.dataset.updatedAt||'';const n=Number(raw);if(Number.isFinite(n)&&n>0)return n;const parsed=Date.parse(raw);return Number.isFinite(parsed)?parsed:0}
