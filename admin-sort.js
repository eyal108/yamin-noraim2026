...
function value(tr,col){
  ...
  if(col===6)return Number(td.dataset.sortDate||0);
...
sortable.forEach(i=>{const h=headers[i];if(!h)return;h.classList.add('sortable');h.tabIndex=0;h.insertAdjacentHTML('beforeend',' <span class="sortMark"></span>');const go=()=>{if(active===i)dir*=-1;else{active=i;dir=i===6?-1:1}sortRows()};h.addEventListener('click',go);h.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();go()}})});
observe();
})();
