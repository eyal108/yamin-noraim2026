...
function buildSnapshot(){
 ...
 for(const b of buckets.values()){
  ...
  const anchor={section:b.section,row:first.row,row_label:first.row_label,block:first.block,segment:first.segment,order_center:Number(avg.toFixed(3)),x_ratio:Number(x.toFixed(4)),zone:x<.34?'start':x>.66?'end':'center',side_ratio:Number(within.toFixed(4)),side:within<.34?'start':within>.66?'end':'center',seat_count:seats.length,seat_ids:seats.map(s=>s.id)};
