...
function mount(){
 ...
 $('referenceImportBtn').onclick=()=>{if(S.editSession)return alert('יש טיוטת שיבוץ פתוחה. שמור אותה או חזור למצב הקודם לפני טעינת תרשים ייחוס.');$('referenceFileInput').click()};
 $('referenceTemplateBtn').onclick=downloadExcelTemplate;$('referenceFileInput').onchange=async e=>{const f=e.target.files?.[0];e.target.value='';if(f)await importExcel(f)};
 renderControl();
}
...
function downloadExcelTemplate(){
 if(!window.XLSX)return alert('רכיב Excel עדיין לא נטען. רענן את העמוד ונסה שוב.');
 const wb=XLSX.utils.book_new(),main=[['משפחה','צד','שורה','אזור','מיקום בתוך האזור','מספר מקומות'],['כהן','גברים',3,'ימין','מרכז',4],['לוי','נשים',5,'מרכז','שמאל',3],['ישראלי','גברים',7,'שמאל','ימין',2],['ישראלי','גברים',8,'שמאל','מרכז',2]],ws=XLSX.utils.aoa_to_sheet(main);
 ws['!cols']=[{wch:22},{wch:12},{wch:10},{wch:12},{wch:20},{wch:16}];XLSX.utils.book_append_sheet(wb,ws,'תרשים ייחוס');
 const help=XLSX.utils.aoa_to_sheet([['עמודה','חובה','ערכים / הסבר'],['משפחה','כן','שם המשפחה כפי שיופיע ברישום.'],['צד','כן','גברים או נשים.'],['שורה','כן','מספר שורה חיובי, לדוגמה 1, 2, 3.'],['אזור','כן','ימין / מרכז / שמאל.'],['מיקום בתוך האזור','לא','ימין / מרכז / שמאל. ברירת המחדל היא מרכז.'],['מספר מקומות','לא','מספר המקומות בקבוצה. ברירת המחדל היא 1.'],['פיצול משפחה','','אם משפחה ישבה בכמה קבוצות, מוסיפים שורה נפרדת לכל קבוצה.'],['הערה','','עמודות נוספות בקובץ מותרות והמערכת תתעלם מהן.']]);help['!cols']=[{wch:22},{wch:10},{wch:70}];XLSX.utils.book_append_sheet(wb,help,'הוראות');
 XLSX.writeFile(wb,'תבנית-תרשים-ייחוס.xlsx');
}
function rowMap(row){const m=new Map();for(const [k,v] of Object.entries(row||{}))m.set(hnorm(k),v);return m}
function pick(m,names){for(const n of names){const k=hnorm(n);if(m.has(k))return m.get(k)}return''}
function parseSection(v){const x=hnorm(v);if(['גברים','גבר','men','male','m'].includes(x))return'men';if(['נשים','אשה','אישה','women','female','w'].includes(x))return'women';return null}
function parsePos(v,allowBlank=false){const x=hnorm(v);if(!x&&allowBlank)return'center';if(['ימין','right','start'].includes(x))return'right';if(['מרכז','center','centre','middle'].includes(x))return'center';if(['שמאל','left','end'].includes(x))return'left';return null}
function ratioFrom(zone,side){const a={right:0,center:1,left:2},z=a[zone],s=a[side];return Number(((z*3+s)/8).toFixed(4))}
function parseExcelRows(rows){
 const famMap=new Map(),errors=[],reqByName=new Map(S.reqs.map(r=>[norm(r.family_name),r.family_id]));
 rows.forEach((row,i)=>{const m=rowMap(row),family=String(pick(m,['משפחה','שם משפחה','family','family name','family_name'])||'').trim();if(!family)return;const section=parseSection(pick(m,['צד','section','side'])),rowValue=pick(m,['שורה','row']),zone=parsePos(pick(m,['אזור','zone'])),side=parsePos(pick(m,['מיקום בתוך האזור','מיקום','side within zone','position','side']),true),countRaw=pick(m,['מספר מקומות','מקומות','seat count','seat_count','count']);const rowNum=Number(rowValue),count=countRaw===''||countRaw==null?1:Number(countRaw);const n=i+2;
  if(!section)errors.push(`שורה ${n}: צד חייב להיות גברים או נשים.`);if(!Number.isInteger(rowNum)||rowNum<1)errors.push(`שורה ${n}: שורה חייבת להיות מספר שלם חיובי.`);if(!zone)errors.push(`שורה ${n}: אזור חייב להיות ימין, מרכז או שמאל.`);if(!side)errors.push(`שורה ${n}: מיקום בתוך האזור חייב להיות ימין, מרכז או שמאל.`);if(!Number.isInteger(count)||count<1||count>100)errors.push(`שורה ${n}: מספר מקומות חייב להיות מספר שלם בין 1 ל־100.`);if(!section||!Number.isInteger(rowNum)||rowNum<1||!zone||!side||!Number.isInteger(count)||count<1||count>100)return;
  const key=norm(family);if(!famMap.has(key))famMap.set(key,{family_id:reqByName.get(key)||null,family_name:family,reference:[]});famMap.get(key).reference.push({section,row:rowNum,row_label:String(rowNum),zone,side,x_ratio:ratioFrom(zone,side),seat_count:count});
 });
 return{families:[...famMap.values()],errors};
}
async function importExcel(file){
 if(S.editSession)return alert('יש טיוטת שיבוץ פתוחה. שמור אותה או חזור למצב הקודם לפני טעינת תרשים ייחוס.');if(!window.XLSX)return alert('רכיב Excel עדיין לא נטען. רענן את העמוד ונסה שוב.');
 try{
  const buf=await file.arrayBuffer(),wb=XLSX.read(buf,{type:'array'}),sheet=wb.Sheets['תרשים ייחוס']||wb.Sheets[wb.SheetNames[0]];if(!sheet)throw Error('לא נמצא גיליון בקובץ.');const rows=XLSX.utils.sheet_to_json(sheet,{defval:'',raw:true});if(!rows.length)throw Error('הגיליון ריק.');
  const headerSet=new Set(Object.keys(rows[0]||{}).map(hnorm)),required=[['משפחה','family'],['צד','section'],['שורה','row'],['אזור','zone']];for(const aliases of required)if(!aliases.some(x=>headerSet.has(hnorm(x))))throw Error(`חסרה עמודה חובה: ${aliases[0]}.`);
  const parsed=parseExcelRows(rows);if(parsed.errors.length)throw Error(parsed.errors.slice(0,10).join('\n')+(parsed.errors.length>10?`\nועוד ${parsed.errors.length-10} שגיאות.`:''));if(!parsed.families.length)throw Error('לא נמצאו משפחות תקינות בקובץ.');
  const base=file.name.replace(/\.(xlsx|xls)$/i,''),name=prompt('שם לתרשים הייחוס:',base||'תרשים ייחוס מ-Excel');if(!name?.trim())return;const snapshot={version:2,source:'excel-import',format:'reference-chart-v1',created_at:new Date().toISOString(),file_name:file.name,families:parsed.families};
  const row={name:name.trim(),source_request_list_id:null,source_layout_id:null,snapshot,is_active:false,created_by:S.session?.user?.email||null,updated_at:new Date().toISOString()},ins=await db.from('yamim_noraim_reference_charts').insert(row).select('id').single();if(ins.error)throw ins.error;const act=await db.rpc('set_yamim_noraim_reference_chart',{p_reference_id:ins.data.id});if(act.error)throw Error('הקובץ נשמר, אך לא ניתן היה להגדיר אותו כפעיל: '+act.error.message);selectedId=ins.data.id;loaded=false;await ensureLoaded();alert(`תרשים הייחוס נטען מ-Excel והוגדר כפעיל. נטענו ${parsed.families.length} משפחות.`);
 }catch(e){alert('טעינת תרשים הייחוס נכשלה:\n'+(e?.message||e))}
}
