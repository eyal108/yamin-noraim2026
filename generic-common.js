...
function makeClient(){const c=rawClient();return new Proxy(c,{get(target,prop){if(prop==='from')return table=>{const mapped=table==='yamim_noraim_admins'?'yamim_noraim_synagogue_admins':table;return wrappedRelation(target.from(mapped),mapped)};const v=target[prop];return typeof v==='function'?v.bind(target):v}})}
