...
async function prepareVisuals(){await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));await sleep(100)}
function makeSheet(){
 ...
 let f=null;if(front){f=front.cloneNode(true);f.removeAttribute('id');sheet.appendChild(f)}
 const c=cleanClone(map.cloneNode(true));c.removeAttribute('id');sheet.appendChild(c);document.body.appendChild(sheet);
 const contentWidth=Math.max(c.scrollWidth||0,f?.scrollWidth||0,h.scrollWidth||0,760);sheet.style.width=Math.ceil(contentWidth+40)+'px';
 return sheet
}
async function canvasForSheet(sheet){
 ...
 const wanted=mobile?2.0:2.4,pixelBudget=mobile?9_000_000:16_000_000,maxDimension=mobile?10000:16000;
 const byPixels=Math.sqrt(pixelBudget/pixels),byDimension=Math.min(maxDimension/width,maxDimension/height),scale=Math.max(.8,Math.min(wanted,byPixels,byDimension));
 return await html2canvas(sheet,{backgroundColor:'#ffffff',scale,useCORS:true,logging:false,width,height,windowWidth:Math.max(1600,width+80),windowHeight:Math.max(1000,height+80),scrollX:0,scrollY:0,imageTimeout:8000})
}
...
function pdfBlob(canvas){
 ...
 const aspect=canvas.width/canvas.height;let pageW,pageH;
 if(aspect>=1){pageW=297;pageH=Math.max(120,Math.min(210,pageW/aspect))}else{pageH=297;pageW=Math.max(120,Math.min(210,pageH*aspect))}
 const pdf=new JsPDF({orientation:pageW>=pageH?'landscape':'portrait',unit:'mm',format:[pageW,pageH],compress:true});
 const pw=pdf.internal.pageSize.getWidth(),ph=pdf.internal.pageSize.getHeight(),margin=4,maxW=pw-margin*2,maxH=ph-margin*2,fit=Math.min(maxW/canvas.width,maxH/canvas.height),w=canvas.width*fit,h=canvas.height*fit,x=(pw-w)/2,y=(ph-h)/2;
 const png=canvas.toDataURL('image/png');pdf.addImage(png,'PNG',x,y,w,h,undefined,'FAST');return pdf.output('blob')
}
...
function showMobileResult(blob,name,type,kind){
 ...
 box.innerHTML=`<div class="exportResultCard" role="dialog" aria-modal="true"><div class="exportResultHead"><div><b>${kind==='pdf'?'ה-PDF מוכן':'התמונה מוכנה'}</b><small>${YNGeneric.esc(name)}</small></div><button type="button" class="exportCloseBtn" aria-label="סגור">×</button></div>${kind==='png'?`<img class="exportResultPreview" src="${activeOutputUrl}" alt="תצוגה מקדימה של תרשים ההושבה">`:'<div class="exportPdfReady">PDF מוכן לשמירה, שיתוף או פתיחה</div>'}<div class="exportResultActions">${canShare?'<button type="button" class="btn primary" data-share>שתף / שמור</button>':''}<button type="button" class="btn" data-open>פתח</button><button type="button" class="btn" data-download>הורד</button></div><div class="exportMobileHelp">הקובץ נוצר בפריסת מחשב רחבה וברזולוציה גבוהה. בטלפון מומלץ להשתמש ב„שתף / שמור”.</div></div>`;
...
async function runExport(kind){
 ...
 setBusy(true,kind==='pdf'?'מכין PDF איכותי...':'מכין תמונה איכותית...');let sheet=null;
 try{
  await prepareVisuals();sheet=makeSheet();await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));const canvas=await canvasForSheet(sheet),name=fileBase()+(kind==='pdf'?'.pdf':'.png');
