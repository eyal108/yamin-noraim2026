...
async function canvasForSheet(sheet){
 ...
 const width=Math.max(1,Math.ceil(sheet.scrollWidth)),height=Math.max(1,Math.ceil(sheet.scrollHeight)),pixels=width*height;
 const wanted=pixels>3_500_000?1.6:2.2,pixelBudget=12_000_000,maxDimension=16000;
...
function supportsDownload(){const a=document.createElement('a');return 'download' in a}
function pdfBlob(canvas){
 ...
 const jpeg=canvas.toDataURL('image/jpeg',0.94);pdf.addImage(jpeg,'JPEG',x,y,w,h,undefined,'FAST');return pdf.output('blob')
}
...
function showFallbackResult(blob,name,type,kind){
 ...
 box.innerHTML=`<div class="exportResultCard" role="dialog" aria-modal="true"><div class="exportResultHead"><div><b>${kind==='pdf'?'ה-PDF מוכן':'התמונה מוכנה'}</b><small>${YNGeneric.esc(name)}</small></div><button type="button" class="exportCloseBtn" aria-label="סגור">×</button></div>${kind==='png'?`<img class="exportResultPreview" src="${activeOutputUrl}" alt="תצוגה מקדימה של תרשים ההושבה">`:'<div class="exportPdfReady">הדפדפן לא מאפשר להתחיל הורדה אוטומטית. הקובץ מוכן.</div>'}<div class="exportResultActions">${canShare?'<button type="button" class="btn primary" data-share>שתף / שמור</button>':''}<button type="button" class="btn" data-open>פתח</button><button type="button" class="btn" data-download>הורד</button></div><div class="exportMobileHelp">זה אותו קובץ שהיה נוצר במחשב; החלון הזה מופיע רק מפני שהדפדפן אינו מאפשר הורדה אוטומטית.</div></div>`;
...
async function runExport(kind){
 ...
 try{
  ...
  if(supportsDownload())downloadBlob(blob,name);else showFallbackResult(blob,name,type,kind)
